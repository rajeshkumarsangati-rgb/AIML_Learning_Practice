...
#results_df = pd.DataFrame(results, columns= ["Model","accuracy","precision","recall…results_df = pd.DataFrame(results, columns= ["Model","accuracy","precision","recall","f1"] ))
...
# Baseline: The model makes fairly accurate predictions, but it may struggle with imbalanced classes, leading to lower recall for the minority class.
# Class Weight Balanced: By adjusting the class weights, the model gives more importance to the minority
...
grid_search = GridSearchCV(estimator=an_pipeline,param_grid=parameter_grid,cv=5,scoring=None,n_jobs=-1)
